...
public class ControllerPanel extends JPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (arg0.getSource().equals(startStop)) {
			if (platform == null) {
				...
				if (arguments != null) {
					...
					for (int i = 0; i < arguments.size(); i++) {
						icms.createComponent("transformationAgent" + i,"bin/br/unb/cic/mase/agents/TransformationAgentBDI.class", new CreationInfo(null, arguments.get(i))).getFirstResult();
			...
			{
				case 1: //January
					updatePlatformStatusInfo("<html><p style=\"color:red;font-size:14px;padding-top:6px;\">January</p></html>");
					increaseWaterLevel(2000);
					break;
				case 2: //February
					updatePlatformStatusInfo("<html><p style=\"color:red;font-size:14px;padding-top:6px;\">February</p></html>");
					increaseWaterLevel(2000);
					break;
				case 3: //March
					updatePlatformStatusInfo("<html><p style=\"color:red;font-size:14px;padding-top:6px;\">March</p></html>");
					increaseWaterLevel(2000);
					break;
				case 4: //April
					updatePlatformStatusInfo("<html><p style=\"color:red;font-size:14px;padding-top:6px;\">April</p></html>");
					increaseWaterLevel(2000);
					break;
				case 5: //May
					...
					break;
				case 6: //June
					updatePlatformStatusInfo("<html><p style=\"color:red;font-size:14px;padding-top:6px;\">June</p></html>");
					increaseWaterLevel(300);
					break;
				case 7: //July
					updatePlatformStatusInfo("<html><p style=\"color:red;font-size:14px;padding-top:6px;\">July</p></html>");
					increaseWaterLevel(300);
					break;
				case 8: //August
					updatePlatformStatusInfo("<html><p style=\"color:red;font-size:14px;padding-top:6px;\">August</p></html>");
					increaseWaterLevel(300);
					break;
				case 9: //September
					updatePlatformStatusInfo("<html><p style=\"color:red;font-size:14px;padding-top:6px;\">September</p></html>");
					increaseWaterLevel(300);
					break;
				case 10: //October
					updatePlatformStatusInfo("<html><p style=\"color:red;font-size:14px;padding-top:6px;\">October</p></html>");
					increaseWaterLevel(300);
					break;
				case 11: //November
					updatePlatformStatusInfo("<html><p style=\"color:red;font-size:14px;padding-top:6px;\">November</p></html>");
					increaseWaterLevel(300);
					break;
				default: //December
					updatePlatformStatusInfo("<html><p style=\"color:red;font-size:14px;padding-top:6px;\">December</p></html>");
					increaseWaterLevel(2000);
					break;
			}
			
			Collection<IDeliberately> serviceTransformationAgent = SServiceProvider.getServices(platform, IDeliberately.class, RequiredServiceInfo.SCOPE_GLOBAL).get();
			for(IDeliberately actualService:serviceTransformationAgent){
				neighbourhood = checkNeighbourhood(actualService.getPositionX(), actualService.getPositionY());
				actualService.deliberate(drySeason, neighbourhood);
			}
			for(IDeliberately actualService:serviceTransformationAgent){
				updateSpaces (actualService.getPositionX(), 
								actualService.getPositionY(),
									actualService.getType());
			}
	...
	private int checkNeighbourhood (int positionX, int positionY)
	{
		int value = 0;
		...
		for (int i=-1; i<2; i++)
		{
			for (int j=-1; j<2; j++)
			{
				if ( (positionX+i < rows) && (positionY+j < columns) &&
						(positionX+i >= 0) && (positionY+j >= 0) && 
							(positionX!=positionX+i || positionY!=positionY+j))
				{
					if (spaces[positionX+i][positionY+j] == 2 ||
							spaces[positionX+i][positionY+j] == 4 ||
								spaces[positionX+i][positionY+j] == 6) //SE FOR COOLABORATIVO, TEM QUE MUDAR ISSO DEPOIS!
					{
						//System.out.println("Spaces[" + (positionX+i) + "][" + (positionY+j) + "]");
						value++;
					}
					else if (spaces[positionX+i][positionY+j]!=0)
					{
						value--;
					}
				}
	...
	private void updateCurrentStepInfo() {
		currentStepInfo.setText(("<html><p style=\"font-size:14px;padding-top:6px;\">Current Step: "+ currentStep + "</p></html>"));
	...
	private void configureGrid() {
		int rows = Integer.parseInt(
		           JOptionPane.showInputDialog("rows: "));
		int columns = Integer.parseInt(
		           JOptionPane.showInputDialog("columns: "));
